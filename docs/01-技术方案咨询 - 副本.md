...
#### 源端 PostgreSQL 环境检查
- **版本检查**：支持 PostgreSQL 11.x 和 12.x（数据传输服务要求）
- **日志级别检查**：
  ```sql
  SHOW wal_level;
  ```
...
#### PostgreSQL 到 OceanBase Oracle 租户类型映射

| PostgreSQL 类型 | OceanBase Oracle 类型 | 注意事项 |
|----------------|---------------------|----------|
| SMALLINT | NUMBER(5,0) | 2字节整数 |
| INTEGER | NUMBER(10,0) | 4字节整数 |
| BIGINT | NUMBER(19,0) | 8字节整数 |
| NUMERIC(p,s) | NUMBER(p,s) | 需保持精度一致 |
| REAL | BINARY_FLOAT | 单精度浮点 |
| DOUBLE PRECISION | BINARY_DOUBLE | 双精度浮点 |
| TEXT | CLOB | 大文本数据 |
| VARCHAR(n) | VARCHAR2(n) | 长度需换算为字节 |
| CHAR(n) | CHAR(n) | 长度需换算为字节 |
| BYTEA | BLOB | 二进制数据 |
| DATE | DATE | 日期类型 |
| TIMESTAMP | TIMESTAMP | 时间戳 |
| TIMESTAMP WITH TIME ZONE | TIMESTAMP WITH TIME ZONE | 带时区 |
| BOOLEAN | NUMBER(1,0) | 0=FALSE, 1=TRUE |
| JSON | CLOB | 需应用层转换 |
| JSONB | CLOB | 需应用层转换 |
| ARRAY | 不直接支持 | 需转换为 CLOB 或拆分表 |
...
- 仅支持 ASCII 码字符
- 不允许包含特殊字符：换行、空格、.|"'`()=;/&\\
- 目标端表名、字段名会自动转换为大写（Oracle 租户）

**REPLICA IDENTITY 配置**（增量同步必需）：
```sql
-- 方式1：确保表有主键（推荐）
...
| 策略 | 适用场景 | 优点 | 缺点 |
|------|---------|------|------|
| **全量迁移** | 数据量小、可接受停机 | 实施简单、风险低 | 业务中断时间长 |
| **增量迁移** | 数据量大、需在线迁移 | 业务中断时间短 | 实施复杂、需额外配置 |
| **全量+增量** | 生产环境、数据量大 | 平衡性能和停机时间 | 需要较长准备期 |

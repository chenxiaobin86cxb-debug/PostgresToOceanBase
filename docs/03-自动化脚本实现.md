...
```yaml
# PostgreSQL 到 OceanBase MySQL 类型映射
postgres_to_mysql:
  smallint: SMALLINT
  integer: INT
  bigint: BIGINT
...
# PostgreSQL 到 OceanBase Oracle 类型映射
postgres_to_oracle:
  smallint: NUMBER(5,0)
  integer: NUMBER(10,0)
  bigint: NUMBER(19,0)
  serial: NUMBER(10,0)  # 需要手动创建序列
  bigserial: NUMBER(19,0)  # 需要手动创建序列
...
class OceanBaseClient:
    ...
    def __init__(self, connection_manager, mode: str = 'mysql'):
        self.conn_mgr = connection_manager
        self.mode = mode
...
class TypeConverter:
    ...
    def convert_column_type(self, postgres_type: str, column_info: Dict, 
                          target_mode: str = 'mysql') -> str:
        """转换列类型"""
        mapping_key = f"postgres_to_{target_mode}"
        type_mapping = self.mapping.get(mapping_key, {})
    ...
    def convert_value(self, value: Any, postgres_type: str, 
                    target_mode: str = 'mysql') -> Any:
        """转换数据值"""
        if value is None:
            return None
    ...
    def migrate_schema(self, tables: List[str], schema: str = 'public',
        ...
        for table_name in tables:
            try:
                ...
                # 生成 CREATE TABLE SQL
                create_sql = self.generate_create_table_sql(pg_schema, target_mode)
...
class DataMigrator:
        ...
        while migrated < total_count:
            try:
                ...
                data = self.pg_client.get_table_data(
                    table_name, schema, migrated, chunk_size
                )
                ...
                for row in data:
                    ...
                    for col_name, value in row.items():
                        # 获取列类型（简化处理）
                        postgres_type = 'text'  # 实际应从表结构获取
                        converted_value = self.converter.convert_value(
                            value, postgres_type, target_mode
                        )
    ...
    def migrate_all_data(self, tables: List[str], schema: str = 'public',
        ...
        for table_name in tables:
            result = self.migrate_table_data(table_name, schema, target_mode)
...
class DataValidator:
    ...
    def validate_checksum(self, table_name: str, schema: str = 'public',
        ...
        """验证数据校验和"""
        # PostgreSQL 采样
        pg_data = self.pg_client.get_table_data(table_name, schema, 0, sample_size)
        pg_checksum = self._calculate_checksum(pg_data)
        
        # OceanBase 采样
        ob_data = self.ob_client.get_table_data(table_name, 0, sample_size)
    ...
    def validate_all(self, tables: List[str], schema: str = 'public') -> Dict:
        """验证所有表"""
        results = {
            'count_validation': [],
            'checksum_validation': []
        ...
        for table_name in tables:
            ...
            if check_checksum:
                result = self.validate_checksum(table_name, schema, sample_size)
...
def main():
    ...
    try:
        ...
        ob_client = OceanBaseClient(conn_mgr, target_config.get('mode', 'mysql'))
        ...
        if not args.data_only and schema_config.get('enabled', True):
            ...
            schema_results = schema_migrator.migrate_schema(
                tables, schema_name, target_config.get('mode', 'mysql')
            )
        ...
        if args.validate and migration_config.get('validation', {}).get('enabled', True):
            ...
            validation_results = validator.validate_all(tables, schema_name)
...
对于 PostgreSQL 的 JSON/JSONB 和数组类型，需要在 `TypeConverter` 中添加特殊处理：
...
tracker.start(table_name, total_count)

# 更新进度
tracker.update(migrated)
...
def test_convert_boolean():
    converter = TypeConverter('config/type_mapping.yaml')
    assert converter.convert_value(True, 'boolean', 'mysql') == 1
    assert converter.convert_value(False, 'boolean', 'mysql') == 0


def test_convert_array():
    converter = TypeConverter('config/type_mapping.yaml')
    assert converter.convert_value([1, 2, 3], 'int[]', 'mysql') == '1,2,3'


def test_convert_json():
    converter = TypeConverter('config/type_mapping.yaml')
    json_value = {"name": "Alice", "age": 30}
    result = converter.convert_value(json_value, 'json', 'oracle')
...
1. **数据类型兼容性**：确保目标类型能容纳源数据
2. **字符集**：统一使用 UTF-8，避免乱码
3. **事务大小**：大事务可能导致超时，适当拆分
4. **网络带宽**：监控网络使用，避免拥塞
5. **磁盘空间**：预留足够的磁盘空间（2倍数据量）
6. **备份**：迁移前备份源数据库
7. **测试**：先在测试环境验证
...
**文档版本**：v1.0  
**更新日期**：2025-01-12  
**适用范围**：PostgreSQL → OceanBase 迁移脚本开发
